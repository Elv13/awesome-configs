    ...
    for _, k in ipairs(keys) do
        ...
        if type(v) == "table" then
            ...
            if margins[v] then
                ...
                bounds.width = bounds.width - (margins[v].top or 0) - (margins[v].bottom or 0)
            end
            ...
            for _, v in ipairs(g) do
                v.y = v.y + y
                v.x = v.x + (margins[v] and (margins[v].top and margins[v].top or 0) or 0)
                table.insert(geometries, v)
            end
            bounds = g.free
            if margins[v] then
                y = y + g.free.y + (margins[v].bottom or 0)
                bounds.height = bounds.height - (margins[v].bottom or 0) - (margins[v].top or 0)
            else
                y = y + g.free.y
            end
        elseif type(v) == "widget" then
            ...
            if v.visible then
                ...
                if margins[v] then
                    ...
                    g.width = g.width + (margins[v].top or 0) + (margins[v].bottom or 0)
                end
            else
                g = {
                    height  = 0,

...
local wiboxes2 = {"top","bottom","left","right"}
wiboxes2["top"] = {}
wiboxes2["bottom"] = {}
wiboxes2["left"] = {}
wiboxes2["right"] = {}
...
function get_offset(position)
  local offset =0
  for _,wprop in ipairs(wiboxes2[position]) do
    --if wprop.position == position then --Actually useless, but once wiboxes2 can be removed, will help
      if wprop.visible == true then
	offset = offset + wprop.height
      end
    --end
  end
  return offset
...
function set_position(wibox, position, screen)
    ...
    local offset = get_offset(position)
    ...
    elseif position == "bottom" then
        wibox.y = (area.y + area.height) - (wibox.height + 2 * wibox.border_width) - offset
    elseif position == "top" then
        wibox.y = area.y + offset
    end
    ...
    for _, wprop in ipairs(wiboxes) do
        if wprop.wibox == wibox then
            ...
            elseif wprop.position == "top" then
                wibox:struts { left = 0, right = 0, bottom = 0, top = get_offset(wprop.position) }
            elseif wprop.position == "bottom" then
                wibox:struts { left = 0, right = 0, bottom = get_offset(wprop.position), top = 0 }
            elseif wprop.position == "left" then
                wibox:struts { left = get_offset(wprop.position), right = 0, bottom = 0, top = 0 }
            elseif wprop.position == "right" then
                wibox:struts { left = 0, right = get_offset(wprop.position), bottom = 0, top = 0 }
            end
            break
        end
    end
...
function new(arg)
    ...
    if position == "top" then
      table.insert(wiboxes2["top"],w)
    elseif position == "bottom" then
      table.insert(wiboxes2["bottom"],w)
    elseif position == "left" then
      table.insert(wiboxes2["left"],w)
    elseif position == "right" then
      table.insert(wiboxes2["right"],w)
    end
